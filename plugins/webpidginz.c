...
#define WEBPIDGIN_VERSION "2.0-Z-2"
 ...
 * 		- Sonidos 		
 *		- Configurar colores 		
 		- mejorar la cuenta de mensajes
 		- añadir ajax a mas cosas como las ventanas activas y la buddy list
 *
 *
 * CHANGES:
 *     TODO:
...
static int gAjaxRefreshTime = 1000;
static gboolean gGroupMessages = 0;
...
static void webpidgin_load_pref()
{
    ...
    gAjaxRefreshTime= purple_prefs_get_int("/plugins/webpidgin/ajax_refresh_msec");
    ...
    for (gnode = purple_get_blist()->root; gnode != NULL; gnode = gnode->next)
    {
        ...
        for (cnode = gnode->child; cnode != NULL; cnode = cnode->next)
        {
            ...
            for (bnode = cnode->child; bnode != NULL; bnode = bnode->next)
            {
				...
				///purple_debug_info("WebPidgin 2/DFG","%s %s %s\n",buddy->name,name, buddy->account->username);
...
static void client_write_header( webpidgin_client_t * httpd, const char *update )
{
    ...
    client_write( httpd,"<style type='text/css'>a {color:#01E} .msgme{color:#55D} .msgother{color:#D55}\n");
    client_write( httpd,".buddyin1{color:#66E} .buddyin2{color:#666} </style>");
    ...
    if( gOptionWWWFrames )
    {
        if( ( strcmp(update,"/") == 0 ) || ( strcmp(update,"/ActiveList") == 0 ) )
        {
            ...
            g_snprintf(buffer,sizeof(buffer),"CMD: <A HREF=%s target=\"list\">U</A> | <A HREF=/Accounts target=\"conv\">A</A> | <A HREF=/Options target=\"conv\">O</A> | <A HREF=/Help target=\"conv\">?</A><HR>\n",update);
        }
        else
        {
    ...
    else
    {
        ...
        g_snprintf(buffer,sizeof(buffer),"CMD: <A HREF=%s>U</A> | <A HREF=/>H</A> | <A HREF=/Accounts>A</A> | <A HREF=/Options>O</A> | <A HREF=/Help>?</A><HR>\n",update);
    }
...
//    PurpleBlistNode *gnode, *cnode, *bnode;
    ...
    for (cnv = purple_get_conversations(); cnv != NULL; cnv = cnv->next) 
    {
		...
		encoded_title = webpidgin_encode( title );
        ...
        unread = g_list_length (conv->message_history);

		if (gGroupMessages)
			g_snprintf(extra3, sizeof(extra3), "#isend");

        g_snprintf(buffer,sizeof(buffer),"&nbsp;&nbsp; <span %s><A HREF=\"conversation?%sid=%p%s\" %s title=\"%s\" %s>%s</A> (%d)\n", extra, time_stamp(), conv, extra3, extra2, name, extra_html, title, unread);
        client_write(httpd, buffer);
...
static int action_options( webpidgin_client_t * httpd, const char * extra )
{
    ...
    if( extra != NULL && (strlen(extra) > 0 ) )
    {
        ...
        value = webpidgin_get_param_int( extra,"ajax_refresh_msec");
        if( value >=1000 && value <= 20000 )
        {
            webpidgin_save_pref_int( "ajax_refresh_msec", value );
        }
    ...
    g_snprintf(buffer,1024,"&nbsp;&nbsp;<input type=checkbox name=use_meta_refresh %s>Auto-refresh  list/status<BR>\n",gOptionMetaRefresh ? "checked" : "");
    client_write(httpd,buffer);
    ...
    g_snprintf(buffer,1024,"&nbsp;&nbsp;<input type=checkbox name=show_no_auth_sessions %s>Show not authenticated sessions<BR>\n",gShowNoAuthSessions ? "checked" : "");
    client_write(httpd,buffer);
    ...
    g_snprintf(buffer,1024,"&nbsp;&nbsp;&nbsp;&nbsp;Milliseconds<input type=text value=\"%d\" size=5 maxlength=5 name=ajax_refresh_msec><BR>\n",gAjaxRefreshTime);
    client_write(httpd,buffer);
    
    g_snprintf(buffer,1024,"&nbsp;&nbsp;&nbsp;&nbsp;Trim conversations after<input type=text value=\"%d\" size=5 maxlength=5 name=max_messages>messages<BR>\n",uOptionTrim);
    client_write(httpd,buffer);
    
    g_snprintf(buffer,1024,"&nbsp;&nbsp;<input type=checkbox name=group_messages %s>Group messages from the same user in conversations<BR>\n", gGroupMessages ? "checked" : "");
    client_write(httpd,buffer);
...
static void show_conversation ( webpidgin_client_t * httpd, PurpleConversation * conv)
{
    ...
    if (purple_conversation_get_type(conv) == PURPLE_CONV_TYPE_CHAT) 
    {
        ...
        g_snprintf(buffer,1024,"&nbsp;%s<BR>", purple_conv_chat_get_topic(chat));
        ...
        for (iter=purple_conv_chat_get_users(chat);iter!=NULL;iter=iter->next) 
        {
            ...
            g_snprintf(buffer,1024,"&nbsp;%s ,", buddy->name);
            client_write(httpd, buffer);
        }
    }
    client_write(httpd, "<HR>");
    ...
    while (iter!=NULL && msgCount<uOptionTrim)
    {
        ...
        if (gOptionUseColor) 
        {
			...
			if (strcasecmp(msg->who, msg->alias) == 0) { //HACK, ITS FAR FROM PERFECT BUT IT WORKS FOR ME
                g_snprintf(color,sizeof(color),"msgme");                  
            } else {
				g_snprintf(color,sizeof(color),"msgother");					
            }
        }
		...
		if (!gGroupMessages || g_ascii_strcasecmp (lastDate, date) || g_ascii_strcasecmp (nick, lastNick))
	    {	       	
        	...
        	g_snprintf(buffer,sizeof(buffer),"<span class='%s'>%s</span>", color, nick);
        	client_write(httpd,buffer);
		...
		// THE MESSAGE                
        {
            ...
            char * tmpstr2 = purple_strreplace (msg->what, "\n", "{{{:::BR:::}}}");
			char * tmpstr = purple_markup_strip_html (tmpstr2);
			g_free(tmpstr2);
...
static int action_conversation( webpidgin_client_t * httpd, const char * extra )
{
	...
	if (gGroupMessages)
    	g_snprintf(buffer,sizeof(buffer),"/conversation?%s%s#isend",time_stamp(),extra);
    else
    	g_snprintf(buffer,sizeof(buffer),"/conversation?%s%s",time_stamp(),extra);
    ...
    if (conv) 
    {
		g_snprintf(buffer,sizeof(buffer),"%s (%s)", purple_conversation_get_title(conv), purple_conversation_get_name(conv));
        client_write(httpd, buffer);
    }else 
    {
		...
		if (buddy)
		{
			g_snprintf(buffer,sizeof(buffer),"<b>%s</b><br/>\n<i>( <u>%s</u> <-> %s )</i>", get_buddy_name(buddy), (buddy->account)->username ,name);        
			client_write(httpd, buffer);
		}
    }
    client_write(httpd, "<BR>");
    ...
    g_snprintf(buffer,sizeof(buffer), "%s <div>\n <input style=\"-wap-input-format: *m\" id=\"imsg\" name=\"%s\" tabindex=\"1\" size=\"48\" maxlength=\"1280\" />\n",form, name);    
    g_snprintf(form,sizeof(form), "%s <input type=\"submit\" id=\"isend\" value=\"Send\" tabindex=\"2\" />\n", buffer);    
    g_snprintf(buffer,sizeof(buffer), "%s </div>\n </form>\n", form);
    g_snprintf(form,sizeof(form), "%s", buffer);
	...
	client_write(httpd,"<div id='divConv'>\n");
    if (conv) 
    {
    ...
    if (gUseJavascript)
    {
		client_write(httpd,"<script type='text/javascript'>var sending=false; \n \
			var countm='';var reqU = new XMLHttpRequest(); var reqsend = new XMLHttpRequest(); var cola=new Array();\n\n");
		...
		client_write(httpd,"\nfunction get(req, page, fready){ try {\
			...
			req.send();	} catch (err) { /*alert (err);*/ } return false; }\n");
		...
		g_snprintf(buffer,sizeof(buffer),"\nfunction true_send(msg){ if (sending){setTimeout('true_send()', 100+Math.random()*10); return}; \n\
			sending=true; if (!msg && cola.length>0) {msg=cola.shift();} \
			if (msg) get(reqsend, 'sendMessage?%s=' + msg, 'if (req.status!=200) showmsg(\"<b>Error sending: '+ msg +'</b><br/>\", \"divError\"); true_send(); sending=false; update();');  \
			else sending=false; }\n" , purple_url_encode(name));
		client_write(httpd, buffer);
		
		g_snprintf(buffer,sizeof(buffer), "\nfunction update() { \
			if (cola.length > 0) showmsg ('Pending messages in queue: ' + cola.length, 'divInfo', true); else showmsg ('', 'divInfo', true); \
			if (reqU.readyState>0 && reqU.readyState<4) return;\
			get(reqU, 'conversationSimple?%s&count', 'if(req.status==200) update1();')}\n", purple_url_encode(name)); 
		client_write(httpd, buffer);

		g_snprintf(buffer,sizeof(buffer), "\nfunction update1() \
			{	var tmp = reqU.responseText; \
			if (tmp != countm) { countm = tmp; get(reqU, 'conversationSimple?%s', 'if(req.status==200) update2();'); }}\n", purple_url_encode(name));
		client_write(httpd, buffer);
				
		client_write(httpd,"function update2() { var divC=document.getElementById('divConv'); divC.innerHTML=reqU.responseText;\
			var msg=document.getElementById('isend'); window.scrollTo(0, msg.offsetTop); }\n");
				
		client_write(httpd, "\nfunction send(){ \n var msg=document.getElementById('imsg'); \
			var nmsg= msg.value; msg.value=''; msg.focus(); if (nmsg == '') return false;\
			cola.push(nmsg); true_send(); return false; }\n");
		
		
		g_snprintf(buffer,sizeof(buffer), "setInterval('update()', %d);\n</script>\n", gAjaxRefreshTime);
		client_write(httpd, buffer);

		client_write(httpd,"<script type=\"text/javascript\">document.getElementById('imsg').focus(); setTimeout(\"document.getElementById('imsg').focus()\",1000);</script>\n");
	}
...
static int action_conversation_simple( webpidgin_client_t * httpd, const char * extra )
{
    char buffer[1024];
    char * name;
    PurpleConversation *conv;
    unsigned unread = 0;
    char * count;
    gboolean onlyCount = 0;

    purple_debug_info("WebPidgin 2","conversation::Extra[%s]\n",extra);

	count = strstr(extra,"&count");
	if ( count )
	{
		onlyCount = 1;
		*count= '\0';
    ...
    {    	
    	...
    	{
		    unread = g_list_length (conv->message_history);
		    g_snprintf(buffer, 1024, "%u", unread);
	    	client_write (httpd, buffer);
    	}else    	
    		show_conversation(httpd, conv);
    }else
...
static int action_conversation_redirect( webpidgin_client_t * httpd, PurpleConversation *conv )
{
    ...
    if (gOptionWWWFrames) {
        ...
        client_write_http_header(httpd);
        g_snprintf(buffer, sizeof(buffer), "<html><head> <meta http-equiv=\"Refresh\" content=\"0; url=/conversation?%sid=%p\"></head><body><p>Please follow <a href=/conversation?%sid=%p>link</a>!</p></body></html>\n",time_stamp(),conv,time_stamp(),conv);
        client_write( httpd, buffer);
        client_write_tail(httpd);
    } else {
        //g_snprintf(buffer, sizeof(buffer), "id=%p",conv);
        //return action_conversation(httpd, buffer);
		
		//Redirijo a la conversacion de nuevo
		
		if (gGroupMessages)
			g_snprintf(buffer, sizeof(buffer), "/conversation?%sid=%p#isend",time_stamp(),conv);
		else
			g_snprintf(buffer, sizeof(buffer), "/conversation?%sid=%p",time_stamp(),conv);
...
static int action_sendMessage( webpidgin_client_t * httpd, const char * extra )
{
    ...
    if( message )
    {
        ...
        *message='\0';
		...
		//HACK MIO DE PRUEBA
		if (g_ascii_strcasecmp (normal, "/wp quit") == 0)
			purple_conversation_destroy(c);
		else if (g_ascii_strcasecmp (normal, "/wp showoffline") == 0)
...
static int action_about( webpidgin_client_t * httpd, const char * extra )
{
    ...
    client_write(httpd,"<B>Credits</B>\n");
    ...
    client_write(httpd,"<BR>\n");
    client_write(httpd,"WebPidgin-Z:<BR>\n");
...
static int action_rss( webpidgin_client_t * httpd, const char * unused )
{
	...
	if (my_addr != NULL)
	{
        g_snprintf(buffer,sizeof(buffer),"  <link>http://%s:%s/</link>\n",my_addr,gOptionPort);
        client_write(httpd,buffer);
    } else {
    ...
    for( item = rssHead.next; item != NULL; item = item->next )
    {
        ...
        if (item->what != NULL)
        {
            ...
            {       
                ...
                g_snprintf(buffer,sizeof(buffer),"http://%s:%s/", my_addr, gOptionPort);

                if (item->convName) {
                    char * name = webpidgin_encode(item->convName);
                    if (name) {
                        /** We have the specific link use that instead */
                        g_snprintf(buffer,sizeof(buffer),"http://%s:%s/conversation?%s",my_addr,gOptionPort,name);
                        g_free(name);
                    }
                }
...
static int action_help( webpidgin_client_t * httpd, const char * unused )
{
    ...
    g_snprintf(buffer,1024,"When Enabled the RSS feed is available at http://%s:%s/status.rss<BR>\n", my_addr ? my_addr : "localhost", gOptionPort);
    client_write(httpd,buffer);
...
static webpidgin_parse_t webpidgin_actions[] = {
    ...
    { "/conversationSimple",action_conversation_simple }
};
///****************************************************************************************************///
...
{	
	...
	{		
		...
		if ( !strcmp (session->ip_client, httpd->ip_client) && 
				( session->ip_client_xff == httpd->ip_client_xff  || !strcmp (session->ip_client_xff, httpd->ip_client_xff) ) &&
				( session->user_agent == httpd->user_agent || !strcmp (session->user_agent, httpd->user_agent) ) )
		{
...
static gboolean options_entry_cb(GtkWidget *widget, GdkEventFocus *evt, gpointer data)
{
    ...
    else if (!strcmp(data, "ajax_refresh_msec")) {
        int size = 0;
        const char * str = gtk_entry_get_text(GTK_ENTRY(widget));
        sscanf( str,"%d",&size);
        webpidgin_save_pref_int( (char*)data, size );
...
static GtkWidget * get_config_frame(PurplePlugin *plugin)
{
	...
	sg = gtk_size_group_new(GTK_SIZE_GROUP_HORIZONTAL);
	spin_button = gaim_gtk_prefs_labeled_spin_button(vbox, _("Last sessions to show:"),
			"/plugins/webpidgin/show_n_last_sessions", 1, 10, sg);	

	toggle = gtk_check_button_new_with_mnemonic(_("Show not authenticated sessions"));
    gtk_box_pack_start(GTK_BOX(vbox), toggle, FALSE, FALSE, 0);
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(toggle),
    ...
    sg = gtk_size_group_new(GTK_SIZE_GROUP_HORIZONTAL);
	spin_button = gaim_gtk_prefs_labeled_spin_button(vbox, _("_Refresh time (Milliseconds):"),
			"/plugins/webpidgin/ajax_refresh_msec", 1000, 20000, sg);
	...
	toggle = gtk_check_button_new_with_mnemonic(_("Group messages from the same user in conversations"));
    gtk_box_pack_start(GTK_BOX(vbox), toggle, FALSE, FALSE, 0);
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(toggle),
...
static void init_plugin(PurplePlugin *plugin)
{
    ...
    purple_prefs_add_int("/plugins/webpidgin/ajax_refresh_msec", 1000);
    purple_prefs_add_int("/plugins/webpidgin/max_messages", 100);    
    purple_prefs_add_bool("/plugins/webpidgin/group_messages", 0);
}
